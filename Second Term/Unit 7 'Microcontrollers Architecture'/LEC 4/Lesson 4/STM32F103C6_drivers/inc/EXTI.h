...
typedef struct{
	EXTI_GPIO_Mapping_T EXTI_PIN     ;    //Specifies the external interrupt for GPIO mapping
	...
	uint8_t             IRQ_Enable   ;    //Enable or Disable the EXTI IRQ(That will enable the IRQ mask in EXTI and NVIC)
    									  //This parameter must be set based on @ref IRQ_Enable_define
	void (*P_IRQ_CallBack)(void)     ;    //Set the C Function() which will be called when IRQ occurs
